...
- 离开听阅页后，可在其他页面用底部迷你播放器继续听。到“我的”邀请好友，模拟好友首次登录，查看双方获券效果和优惠购书价。
...
迷你播放器支持在小程序各页面继续播放。本地打包音频不能保证离开小程序后的后台/锁屏播放；代码已为线上 HTTPS 音频预留 `BackgroundAudioManager` 接入与 `requiredBackgroundModes` 配置，正式上线前需要部署音频并真机验证。学习统计来自本机实际操作，不预填虚假成绩。
...
```text
modules/listen-read/
  pages/          首页、书架、我的、图书详情、听阅、小测、单词花园、登录、邀请、优惠券
  components/     可复用图书卡片、迷你播放器
