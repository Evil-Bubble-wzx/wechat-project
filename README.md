...
参考用户提供的 MegaWords 截图制作的原生微信小程序，并已接入本仓库`backend/`的local/test API。当前默认运行正式预发布模式：仅开放 Peter Rabbit 数字听阅，隐藏实体借阅、优惠券、邀请、演示购买和固定验证码等演示入口。完整旧功能仍保留在显式 Demo 模式中，供开发验收。
...
**这是已完成本地X/B联调的预发布工程，不是已部署的线上系统。** 正式模式已具备微信登录客户端、token刷新、内容、Quiz和排行API适配；本机develop环境使用显式stub身份，真实AppID/AppSecret、合法域名和真机验收尚未完成。支付或实体借阅仍不开放。Demo模式的微信/短信登录不调用真实身份服务，固定验证码`123456`仅用于显式Demo；手机号不持久化，演示预约不占真实库存。
...
阅读小测支持彼得兔固定10道项目原创双语理解题，每题绑定审核正文cue，并提供双语解释；离线结果先标为“未经服务端验证”，已登录且API可用时再提交服务端按版本化题库判分。排行榜已接入服务端`quiz-score-v1`：最终指标是0～1000综合算法分，不是完成Quiz的书本数；完成书本数占30%，另结合词量、贝叶斯校正正确率、挑战度、成长和题材广度。页面提供A/B校区、滚动7日、严格自然周、自然月、自然年、具体周/月以及Kindergarten～Grade 9和阅读级别筛选，并可进入参与者Quiz明细；未登录、服务不可用或不足10人时显示真实状态，不编造用户和排名。服务端判题契约以`backend/contracts/openapi.json`为准，算法见`backend/docs/adr/0003-ranking-score-v1.md`，X/B联调记录见`backend/docs/xb-integration-record.md`。
...
音频字段可增加`audioUrl`，应为已确认授权的HTTPS资源。`coverUrl`可配置用于锁屏显示的远程封面。客户端保留本地内容作为离线/预发布回退；登录且API可用时会从服务端读取书目和版本化manifest。后端上传、校验、Worker和原子发布流程已在local/test实现，但内容权利门槛、大批量压测和生产对象存储尚未关闭，因此不能把它理解为已上线的云端内容发布。
...
- `services/api.js`与`config/api.js`：正式账户、内容、Quiz和算法排行的服务端适配；客户端不保存AppSecret。
...
当前正式首版不包含实体借阅；相关页面和规则只保留在显式 Demo 模式中。已被新版本取代或与当前范围冲突的旧规划不再保留，项目级交接见`docs/项目交接说明-2026-09-24.md`。
