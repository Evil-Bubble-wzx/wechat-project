...
function validate(books) {
 ...
 for(const [i,b] of books.entries()){
  ...
  for(const k of ['id','title','zh','author','cover','number','desc','time','category'])if(typeof b[k]!=='string'||!b[k].trim())throw new Error(label+' 缺少 '+k)
  if(![b.series,b.editionLabel].some(value=>typeof value==='string'&&value.trim()))throw new Error(label+' 缺少 series 或 editionLabel')
  if(b.level!==null&&(typeof b.level!=='string'||!b.level.trim()))throw new Error(label+' level 必须为非空字符串或 null')
  if(ids.has(b.id))throw new Error(label+' ID 重复');ids.add(b.id)
  for(const k of ['words','duration','stock'])if(!Number.isFinite(b[k])||b[k]<0)throw new Error(label+' '+k+' 必须为非负数字')
  if(b.pages!==null&&(!Number.isFinite(b.pages)||b.pages<0))throw new Error(label+' pages 必须为非负数字或 null')
