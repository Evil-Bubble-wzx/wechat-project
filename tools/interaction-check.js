...
;(async()=>{
 ...
 await go('quiz','.answer');for(const answer of [0,1,1,2,1,0,0,1,0,1]){await page.locator('.answer').nth(answer).click();await page.locator('[data-action="nextQuestion"]').click();await page.locator('[data-action="nextQuestion"]').click()}assert.ok((await page.locator('.result-big').textContent()).includes('100'))
 ...
 assert.deepEqual(errors,[]);console.log('PASS: explicit Demo login, search, favorites, loans, quiz, audio state, promotions, and all 14 routes at 320/390/430px. No console errors.')
 await browser.close()
