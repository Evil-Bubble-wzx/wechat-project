...
;(async()=>{
 ...
 await go('quiz','.answer');for(const answer of [0,1,2,1,1]){await page.locator('.answer').nth(answer).click();await page.locator('[data-action="nextQuestion"]').click();await page.locator('[data-action="nextQuestion"]').click()}assert.ok((await page.locator('.result-big').textContent()).includes('100'))
 ...
 await go('player?id=peter','.play-button');await page.locator('[data-action="toggleSubtitle"]').click();await page.locator('[data-action="toggleSubtitle"]').click();const savedWord=(await page.locator('.reader-word').first().textContent()).trim();await page.locator('.reader-word').first().click();await page.locator('[data-action="saveWord"]').click();await page.locator('[data-action="speed"]').click();assert.ok((await page.locator('[data-action="speed"]').textContent()).includes('1.25'))
 ...
 for(const width of [320,390,430]){await page.setViewportSize({width,height:844});for(const route of ['home','library','loans','me','detail','player','recent','report','ranking','login','coupons','invite','quiz']){await go(route,'.app');assert.ok(await page.locator('#app').evaluate(el=>el.scrollWidth<=el.clientWidth+1),route+' horizontal overflow at '+width)}}
 await page.setViewportSize({width:390,height:844});for(const route of ['login','library','loans','me','quiz','report','coupons','invite']){await go(route,'.app');await page.screenshot({path:'artifacts/demo/'+route+'-mobile.png'})}
 assert.deepEqual(errors,[]);console.log('PASS: explicit Demo login, search, favorites, loans, quiz, audio state, promotions, and all 13 routes at 320/390/430px. No console errors.')
 await browser.close()
