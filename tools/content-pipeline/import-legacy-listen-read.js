...
const importedCatalog = stories.map((book, bookIndex) => {
  ...
  const chapters = book.chapters.map((chapter, chapterIndex) => {
    ...
    quizzes[chapter.id] = chapter.quiz.map((question, questionIndex) => Object.assign({}, question, {
      audio: question.options.map((_, optionIndex) => {
        const name = `${chapter.id}-${questionIndex}-${optionIndex}.mp3`
        if (!fs.existsSync(path.join(source, 'quiz-audio', name))) throw new Error(`Missing quiz audio: ${name}`)
        const output = path.join(mini, 'quiz/assets/audio', name)
        fs.mkdirSync(path.dirname(output), { recursive: true })
        fs.copyFileSync(path.join(source, 'quiz-audio', name), output)
        return `/quiz/assets/audio/${name}`
      })
    }))
...
fs.writeFileSync(path.join(mini, 'modules/listen-read/legacy-quiz-data.js'), '// Generated by npm run content:legacy. Do not edit by hand.\nmodule.exports = ' + JSON.stringify(quizzes) + '\n')
