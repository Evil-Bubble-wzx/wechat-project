...
let draft = createPrefilledVocabularyReview(candidates)
const completedPath = path.join(reviewDir, 'vocab-decisions.completed.json')
let completedReview = false
if (fs.existsSync(completedPath)) {
  const completed = readJson(completedPath)
  if (validateVocabularyReview(completed, packageDir).valid) {
    draft = completed
    completedReview = true
  }
}
const reboundCues = completedReview
  ? readJson(path.join(packageDir, 'dist', 'cues.json'))
  : bindPendingVocabulary(packageDir, candidates)
writeJson(path.join(workDir, 'vocab.candidates.json'), candidates)
if (!completedReview) writeJson(path.join(reviewDir, 'vocab-decisions.json'), draft)
...
if (!completedReview) {
  ...
  qa.vocabularyReview = {
    ...
    prefilledUnits: draft.counts.prefilledUnits,
    completePrefillUnits: draft.counts.completePrefillUnits,
    splitPrefillUnits: draft.counts.splitPrefillUnits,
    properNounExclusionCandidates: draft.counts.properNounExclusionCandidates,
    sourceCueSha256: candidates.sourceCueSha256
  }
  qa.blockers = [
    `${candidates.counts.reviewUnits} vocabulary review units require editorial approval.`,
...
<section class="panel"><h1>Peter Rabbit 词汇编辑审核</h1><p class="muted">${completedReview ? `已完成授权的 AI 辅助全量审核 · ${draft.entries.length}/${draft.entries.length} 项` : `已机器预填 ${draft.counts.completePrefillUnits}/${draft.counts.reviewUnits} 项 · 尚待确认`} · 英式 IPA 优先</p><p class="warning">${completedReview ? '<b>词汇审核已应用。</b> 审核方式、授权人和低置信度项目已写入审计记录；整体人工完成门槛及发布门槛仍保持关闭。' : '<b>现在不用从空白填写。</b> 请逐项核对预填的 lemma、词性、IPA、语境释义和出现位置，再勾选确认。英文释义是待复核草稿，未达到 100% 不可导出 completed 文件。'}</p><audio id="audio" controls preload="metadata" src="../source/audio-original.mp3"></audio><div class="toolbar"><input class="search" id="search" placeholder="搜索词形、lemma、例句"><select id="risk"><option value="all">全部项目</option><option value="high">有风险标记</option><option value="unmatched">无开放来源匹配</option><option value="split">已拆分多义项</option><option value="exclude">专名排除候选</option><option value="done">已确认</option><option value="todo">未确认</option></select><span id="stats"></span></div><div class="progress"><span id="bar"></span></div></section>
...
const initial=${embedded};const storageKey='tingyue-vocab-review-'+initial.pieceId+'-'+initial.sourceCueSha256+'-'+(initial.prefillVersion||'base');let state=load();let selectedId=state.entries[0]?.id||null;let clipEnd=null;
...
console.log(JSON.stringify({
  ...
  completePrefillUnits: draft.counts.completePrefillUnits,
  completedReview,
  vocabularyApproved: qa.checks.vocabularyApproved,
  publishable: false
}, null, 2))
