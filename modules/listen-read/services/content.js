...
const ozDictionary = require('../content/oz-dictionary')
...
function canRead(bookId, pieceId) { const book = getBook(bookId); return !!book && (book.free || book.chapters[0].id === pieceId || store.owns(bookId)) }
function listBooks() {
...
function chapterData(bookId, pieceId) {
  ...
  return Object.assign({}, ch, { cues, duration: timing.duration, audio: ch.audio || '/assets/audio/' + pieceId + '.mp3' })
}
function meaning(word, bookId) { return (bookId === 'wonderful-wizard-oz' && ozDictionary[word.toLowerCase()]) || dictionary[word.toLowerCase()] || { lemma: word, ph: '', pos: '', zh: '这个词暂未收录', en: 'Try reading the sentence together.' } }
