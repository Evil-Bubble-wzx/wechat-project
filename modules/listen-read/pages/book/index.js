...
Page({
  data: { book: null, chapters: [], owned: false, purchaseOpen: false },
  ...
  refresh() {
    ...
    this.setData({ book, owned, chapters: book.chapters.map((ch, i) => { const c = content.chapterData(book.id, ch.id); return { id: ch.id, title: ch.title, zh: ch.zh, number: '0' + (i+1), free: i === 0, locked: i > 0 && !owned, duration: content.time(c.duration), completed: !!(state.progress[ch.id] || {}).completed } }) })
  },
  ...
  noop() {},
  confirmPurchase() {
    if (store.unlock(this.bookId)) { this.setData({ purchaseOpen: false }); this.refresh(); wx.showToast({ title: '已模拟解锁整本', icon: 'success' }) }
  }
})
