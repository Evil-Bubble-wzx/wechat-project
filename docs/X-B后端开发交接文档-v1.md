...
当前自动基线为 `npm test` 89/89 通过。自动检查不能代替真机、账号、权利、隐私、真实后端联调和上线资格。
...
X 侧已完成 S-01 客户端候选快照基础：显式正式状态输入、严格白名单、可信区间毫秒化、Quiz 降权、生词 `surface_only` 限制、确定性 `s01-v1:<sha256>`、显式同意和单飞 mock 提交。该实现没有真实 API、页面或服务端回执，不得视为 S-01 已完成。B 冻结契约时应复用其安全边界，但请求字段仍以双方确认的 OpenAPI/JSON Schema 为准。
