...
`source/stories.json` 保存英文句子、现有中文译文与旧测验，`source/timings.json` 保存逐句起止时间，两个词典 JSON 保存现有点词释义。`source/audio/` 为旧项目实际播放的七段 MP3，`source/quiz-audio/` 保留 54 段旧测验选项朗读，`source/covers/` 为四张封面。旧测验及选项音频已归档，当前目标项目的测验界面仍只接 Peter Rabbit。`source/original/` 保留 Oz 当时使用的 EPUB 与 OGG 作为来源材料；它们不进入小程序包。各章节的 `source/text-original.txt`、`source/audio-original.mp3`、`source/rights.json`、`dist/cues.json` 和 `dist/manifest.json` 可供内容管线逐章复核，`publishable` 均保持 `false`。
